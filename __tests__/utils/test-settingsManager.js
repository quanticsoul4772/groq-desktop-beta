...
test('loadSettings without app instance returns minimal defaults', () => {
  ...
  assertTrue(
    Object.prototype.hasOwnProperty.call(settings, 'GROQ_API_KEY'),
    'Settings object must contain GROQ_API_KEY property for API authentication'
  );
  assertTrue(
    Object.prototype.hasOwnProperty.call(settings, 'model'),
    'Settings object must contain model property to specify the AI model to use'
  );
  assertEquals(
    settings.temperature,
    0.7,
    `Default temperature must be 0.7 for consistent model behavior, found ${settings.temperature}`
  );
  assertEquals(
    settings.popupEnabled,
    true,
    `Default popupEnabled must be true to enable popup functionality, found ${settings.popupEnabled}`
  );
});
...
test('Boolean false values are preserved in settings merge', () => {
  ...
  assertEquals(
    loadedSettings.popupEnabled,
    false,
    `Settings merge must preserve boolean false value for popupEnabled, found ${loadedSettings.popupEnabled}`
  );
  assertEquals(
    loadedSettings.enableSpellCheck,
    false,
    `Settings merge must preserve boolean false value for enableSpellCheck, found ${loadedSettings.enableSpellCheck}`
  );
...
test('Zero numeric values are preserved in settings merge', () => {
  ...
  assertEquals(
    ...
    0,
    `Settings merge must preserve zero value for temperature parameter, found ${loadedSettings.temperature}`
  );
  assertEquals(
    loadedSettings.top_p,
    0,
    `Settings merge must preserve zero value for top_p parameter, found ${loadedSettings.top_p}`
  );
  assertEquals(
    loadedSettings.toolOutputLimit,
    0,
    `Settings merge must preserve zero value for toolOutputLimit parameter, found ${loadedSettings.toolOutputLimit}`
  );
...
test('Empty string values are preserved in settings merge', () => {
  ...
  assertEquals(
    loadedSettings.customSystemPrompt,
    '',
    `Settings merge must preserve empty string for customSystemPrompt, found '${loadedSettings.customSystemPrompt}'`
  );
  assertEquals(
    loadedSettings.customCompletionUrl,
    '',
    `Settings merge must preserve empty string for customCompletionUrl, found '${loadedSettings.customCompletionUrl}'`
  );
  assertEquals(
    loadedSettings.customApiBaseUrl,
    '',
    `Settings merge must preserve empty string for customApiBaseUrl, found '${loadedSettings.customApiBaseUrl}'`
  );
  assertEquals(
    loadedSettings.theme,
    '',
    `Settings merge must preserve empty string for theme, found '${loadedSettings.theme}'`
  );
...
test('null vs undefined handling in settings', () => {
  ...
  assertEquals(
    loadedSettings.temperature,
    0.7,
    `Null temperature value must fallback to default 0.7 via nullish coalescing, found ${loadedSettings.temperature}`
  );
  assertEquals(
    loadedSettings.top_p,
    0.95,
    `Undefined top_p value must fallback to default 0.95 via nullish coalescing, found ${loadedSettings.top_p}`
  );
  assertEquals(
    loadedSettings.customSystemPrompt,
    '',
    `Null customSystemPrompt must fallback to default empty string, found '${loadedSettings.customSystemPrompt}'`
  );
  assertEquals(
    loadedSettings.popupEnabled,
    true,
    `Null popupEnabled must fallback to default true value, found ${loadedSettings.popupEnabled}`
  );
...
test('Environment variable precedence for API key', () => {
  ...
  assertEquals(
    loadedSettings.GROQ_API_KEY,
    'env-test-key',
    `GROQ_API_KEY from environment must override file setting for security, expected 'env-test-key' but found '${loadedSettings.GROQ_API_KEY}'`
  );
...
test('Settings file creation when missing', () => {
  ...
  assertTrue(
    ...
    `Settings file must be auto-created when missing at path: ${settingsPath}`
  );
  ...
  assertEquals(
    loadedSettings.temperature,
    0.7,
    `Auto-created settings must have default temperature 0.7, found ${loadedSettings.temperature}`
  );
  assertEquals(
    loadedSettings.popupEnabled,
    true,
    `Auto-created settings must have default popupEnabled true, found ${loadedSettings.popupEnabled}`
  );
...
test('Error handling for corrupted JSON file', () => {
  ...
  assertEquals(
    loadedSettings.temperature,
    0.7,
    `Corrupted JSON must fallback to default temperature 0.7 for graceful recovery, found ${loadedSettings.temperature}`
  );
  assertEquals(
    loadedSettings.popupEnabled,
    true,
    `Corrupted JSON must fallback to default popupEnabled true for graceful recovery, found ${loadedSettings.popupEnabled}`
  );
...
test('Settings validation logic', () => {
  ...
  assertEquals(
    result.success,
    false,
    `Settings validation must reject null input for data integrity, result.success was ${result.success}`
  );
  assertTrue(
    result.error.includes('Invalid settings object'),
    `Validation error must specify 'Invalid settings object' for null input, got: '${result.error}'`
  );
  ...
  assertEquals(
    result.success,
    false,
    `Settings validation must reject non-object input (string), result.success was ${result.success}`
  );
  ...
  assertEquals(
    result.success,
    true,
    `Settings validation must accept valid object input, result.success was ${result.success}`
  );
...
test('Mock IPC handlers are registered and can be invoked', () => {
  ...
  assertEquals(
    result.temperature,
    0.8,
    `IPC get-settings handler must return saved temperature value 0.8, found ${result.temperature}`
  );
  assertEquals(
    result.popupEnabled,
    false,
    `IPC get-settings handler must return saved popupEnabled false value, found ${result.popupEnabled}`
  );
  ...
  assertEquals(
    saveResult.success,
    true,
    `IPC save-settings handler must succeed with valid data, saveResult.success was ${saveResult.success}`
  );
  ...
  assertEquals(
    ...
    0.9,
    `IPC handler must persist temperature 0.9 to settings file, found ${savedData.temperature} in saved file`
  );
  assertEquals(
    savedData.popupEnabled,
    true,
    `IPC handler must persist popupEnabled true to settings file, found ${savedData.popupEnabled} in saved file`
  );
...
test('Mock IPC _reset clears all handlers', () => {
  ...
  assertTrue(
    settings !== undefined,
    `IPC get-settings handler must be registered and return defined result, got ${typeof settings}: ${settings}`
  );
  ...
  try {
    ...
    assertTrue(
      ...
      `Mock IPC must throw 'No handler registered' error after reset, got: '${error.message}'`
    );
  }

  assertTrue(
    errorCaught,
    `Mock IPC _reset() must clear handlers causing subsequent calls to throw errors, errorCaught was ${errorCaught}`
  );
...
test('Mock IPC throws error for unregistered handlers', () => {
  ...
  try {
    ...
    errorCaught = true;
    assertTrue(
      error.message.includes('No handler registered for channel: non-existent-channel'),
  ...
  assertTrue(
    errorCaught,
    `Mock IPC must throw error when invoking unregistered handler 'non-existent-channel', errorCaught was ${errorCaught}`
  );
});
