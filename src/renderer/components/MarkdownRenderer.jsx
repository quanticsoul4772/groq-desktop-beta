...
const components = {
  ...
  table: ({ node: _, ...props }) => <table className="table-auto w-full mb-1" {...props} />,
  thead: ({ node: _, ...props }) => (
  ...
  ol: ({ node: _, children, ...props }) => {
    return (
      <ol className="ml-0 mb-1 list-decimal" {...props}>
        {children}
      </ol>
    );
  },
    ...
    if (isImageLink) {
      return (
        <div>
          ...
          <div className="mt-2">
            <img src={href} alt={`Image at ${href}`} className="max-w-full h-auto" />
          </div>
        </div>
      );
    }
