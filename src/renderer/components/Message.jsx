              ...
              {hasExecutedTools && (
                ...
                >
                  ...
                  {isStreamingMessage && currentTools?.some((t) => !t.output) && (
                    <svg
                      className="animate-spin ml-2 h-4 w-4 text-green-600"
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
                    >
            ...
            {showExecutedTools && currentTools?.length > 0 && (
              <div className="space-y-2">
                {currentTools.map((tool, index) => {
                  ...
                  return (
                    <div
                      key={`tool-${tool.index || index}`}
                      className={`p-3 rounded-md text-sm border ${isLive ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'}`}
                    >
                      <div
                        className={`font-semibold mb-2 ${isLive ? 'text-green-800' : 'text-gray-800'}`}
                      >
                        {tool.type === 'python' ? '🐍 Python Code' : `🔧 ${tool.type || 'Tool'}`}
                        {tool.name && (
                          <span
                            className={`ml-2 font-normal text-sm ${isLive ? 'text-green-700' : 'text-gray-700'}`}
                          >
                            ({tool.name})
                          </span>
                        )}
                      ...
                      {tool.arguments && (
                        <div className="mb-2">
                          <div
                            className={`text-xs mb-1 ${isLive ? 'text-green-700' : 'text-gray-700'}`}
                          >
                            Code:
                          </div>
                          <pre
                            className={`p-2 rounded overflow-x-auto text-xs ${isLive ? 'bg-green-100 text-green-900' : 'bg-gray-100 text-gray-900'}`}
                          >
                            ...
                            if (!shouldShowCollapse) {
                              ...
                              return (
                                <div>
                                  ...
                                  <pre
                                    className={`bg-white p-2 rounded overflow-x-auto text-xs border ${isLive ? 'text-green-900 border-green-200' : 'text-gray-900 border-gray-200'}`}
                                  >
                                    {tool.output}
                                  </pre>
                                </div>
                            ...
                            return (
                              <div>
                                <div className="flex items-center justify-between mb-1">
                                  <div
                                    className={`text-xs ${isLive ? 'text-green-700' : 'text-gray-700'}`}
                                  >
                                    Output:
                                  </div>
                                  <button
                                ...
                                {isOutputCollapsed(tool.index || index) ? (
                                  <div
                                    className={`bg-white p-2 rounded text-xs border ${isLive ? 'text-green-700 border-green-200' : 'text-gray-700 border-gray-200'} italic`}
                                  >
