    ...
    try {
      ...
      await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(
            () => reject(new Error(`Health check timed out after ${healthCheckTimeout}ms`)),
            healthCheckTimeout
          )
        ),
      ]);
      ...
      if (mcpClients[serverId] === client) {
        // Ensure we're removing the correct client instance
        ...
        try {
          ...
          console.error(
            `[${serverId}] Error closing client after health check failure: ${closeError.message}`
          );
        }
      } else {
        console.warn(
          `[${serverId}] Client instance mismatch during health check failure cleanup. State might be inconsistent.`
        );
      }
    }
  }, intervalMs);
...
class StaticAuthProvider {
  ...
  get redirectUrl() {
    return 'http://127.0.0.1/callback-dummy';
  }
  get clientMetadata() {
    return { client_name: 'Groq Desktop (Static)', redirect_uris: [this.redirectUrl] };
  }
  async saveClientInformation(_info) {
    console.warn('StaticAuthProvider saveClientInformation called unexpectedly');
  }
  async saveTokens(_tokens) {
    console.warn('StaticAuthProvider saveTokens called unexpectedly');
  }
  async redirectToAuthorization(_url) {
    console.warn('StaticAuthProvider redirectToAuthorization called unexpectedly');
  }
  async saveCodeVerifier(_verifier) {
    console.warn('StaticAuthProvider saveCodeVerifier called unexpectedly');
  }
  async codeVerifier() {
    console.warn('StaticAuthProvider codeVerifier called unexpectedly');
    return 'dummy';
  }
}
...
async function connectMcpServerProcess(serverId, connectionDetails, authProviderInstance = null) {
  ...
  if (mcpClients[serverId]) {
    ...
    if (
      oldClient.transport instanceof StdioClientTransport &&
      oldClient.transport?.process?.stderr
    ) {
    ...
    try {
      ...
      console.warn(`Error closing previous client ${serverId}: ${e.message}`);
    }
    delete mcpClients[serverId];
    discoveredTools = discoveredTools.filter((t) => t.serverId !== serverId);
    delete mcpServerLogs[serverId];
  ...
  if (transportType === 'stdio' && !connectionDetails.command)
    throw new Error(`[${serverId}] Internal Error: Missing command for stdio.`);
  if ((transportType === 'sse' || transportType === 'streamableHttp') && !connectionDetails.url)
    throw new Error(`[${serverId}] Internal Error: Missing url for ${transportType}.`);
  ...
  console.log(
    `Attempting ${transportType.toUpperCase()} connection to ${serverId}... ${authProviderInstance ? '(with auth provider)' : '(without auth provider)'}`
  );
  ...
  try {
    ...
    if (transportType === 'sse') {
      ...
      const combinedPath = [
        ...
        .join(pathSeparator); // Deduplicate and join
  ...
  try {
    ...
    if (transport instanceof StdioClientTransport && transport.stderr) {
      ...
      const handleStderrError = (err) => {
        ...
        mcpServerLogs[serverId] = [...(mcpServerLogs[serverId] || []), errorLine].slice(
          -MAX_LOG_LINES
        );
      ...
      const handleStderrEnd = () => {
        ...
        mcpServerLogs[serverId] = [...(mcpServerLogs[serverId] || []), endLine].slice(
          -MAX_LOG_LINES
        );
      ...
      mcpServerLogs[serverId] = [...(mcpServerLogs[serverId] || []), warnLine].slice(
        -MAX_LOG_LINES
      );
    ...
    const toolsResult = await Promise.race([
      client.listTools(),
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error(`listTools timed out`)), listToolsTimeout)
      ),
    ]);
    ...
    console.error(
      `[${serverId}] Failed to connect or initialize SDK client:`,
      error.message || error
    );
    ...
    if (
      ...
      if (authProviderInstance) {
        ...
        console.warn(
          `[${serverId}] Connection failed without auth provider, assuming authorization required.`
        );
        requiresAuth = true;
      }
    }
...
async function connectConfiguredMcpServers() {
  ...
  try {
    ...
    const serverConfigs = Object.entries(settings.mcpServers).filter(
      ([serverId]) => !disabledServers.includes(serverId)
    );
    ...
    console.log(
      `[MCP AUTO-CONNECT] Found ${totalCount} MCP servers. Connecting to ${enabledCount} enabled (${disabledCount} disabled)...`
    );
    console.log(
      '[MCP AUTO-CONNECT] Enabled servers:',
      serverConfigs.map(([id]) => id)
    );
    ...
    const connectionPromises = serverConfigs.map(async ([serverId, serverConfig]) => {
      try {
        ...
        if (transportType === 'sse' || transportType === 'streamableHttp') {
          if (!serverConfig.url)
            throw new Error(`Missing 'url' for ${transportType.toUpperCase()} server ${serverId}.`);
          try {
            new URL(serverConfig.url);
            connectionDetails.url = serverConfig.url;
          } catch (e) {
            throw new Error(
              `Invalid 'url' for ${transportType.toUpperCase()} ${serverId}: ${e.message}`
            );
          }
          ...
          if (!serverConfig.command)
            throw new Error(`Missing 'command' for stdio server ${serverId}.`);
          ...
          console.log(
            `[MCP AUTO-CONNECT] ${serverId} command: ${connectionDetails.command} ${connectionDetails.args.join(' ')}`
          );
        }
        ...
        if (error instanceof AuthorizationRequiredError) {
          console.warn(
            `[MCP AUTO-CONNECT] ⚠️ [${serverId}] Auto-connect failed due to required authorization. Manual connection/authorization needed.`
          );
          ...
          console.error(
            `[MCP AUTO-CONNECT] ❌ Failed auto-connect ${serverId}:`,
            error.message || error
          );
    ...
    console.error(
      '[MCP AUTO-CONNECT] ❌ Critical error during connectConfiguredMcpServers:',
      error
    );
  }
}
...
function initializeMcpHandlers(ipcMain, app, mainWindow, loadSettings, resolveCommandPath) {
  ...
  ipcMain.handle('connect-mcp-server', async (event, serverConfig) => {
    if (
      !serverConfig ||
      !serverConfig.id ||
      ((serverConfig.transport === 'sse' || serverConfig.transport === 'streamableHttp') &&
        !serverConfig.url) ||
      (!(serverConfig.transport === 'sse' || serverConfig.transport === 'streamableHttp') &&
    ...
    try {
      ...
      if (settings.disabledMcpServers?.includes(id)) {
        ...
        try {
          ...
          console.error(`Failed to save settings after enabling ${id}:`, saveError);
        }
      }
      ...
      if (
        connectionDetails.transport === 'sse' ||
        connectionDetails.transport === 'streamableHttp'
      ) {
        if (!url)
          return {
            success: false,
            error: `Missing 'url' for ${connectionDetails.transport.toUpperCase()} server ${id}`,
            tools: [],
            allTools: discoveredTools,
          };
        try {
          ...
          return {
            success: false,
            error: `Invalid 'url' for ${connectionDetails.transport.toUpperCase()} ${id}: ${e.message}`,
            tools: [],
            allTools: discoveredTools,
          };
        }
        ...
        if (command) {
          ...
          // Handle legacy scriptPath if needed
          const absoluteScriptPath = path.resolve(scriptPath);
          if (!fs.existsSync(absoluteScriptPath))
            return {
              success: false,
              error: `Script not found: ${absoluteScriptPath}`,
              tools: [],
              allTools: discoveredTools,
            };
          ...
          return {
            success: false,
            error: `Internal Error: No command/scriptPath for stdio ${id}.`,
            tools: [],
            allTools: discoveredTools,
          };
        }
    ...
    try {
      ...
      if (!settings.disabledMcpServers.includes(serverId)) {
        ...
        try {
          ...
          console.error(`Failed to save settings after disabling ${serverId}:`, saveError);
        }
      }
...
async function retryConnectionAfterAuth(serverId /* removed accessToken */) {
  ...
  if (!connectionDetails) {
    console.error(
      `[MCPManager][${serverId}] Cannot retry connection: Original connection details not found.`
    );
    ...
    if (mainWindowInstance?.webContents) {
      mainWindowInstance.webContents.send('mcp-auth-reconnect-complete', {
        serverId,
        success,
        error: errorMsg,
      });
  ...
  try {
    ...
    console.log(
      `[MCPManager][${serverId}] Retrieved tokens (token type: ${tokens.token_type}) and client info for retry.`
    );
    ...
    if (error instanceof AuthorizationRequiredError) {
      console.error(
        `[MCPManager][${serverId}] Reconnection failed: Authorization still required even with new token.`
      );
    ...
    console.log(
      `[MCPManager][${serverId}] Sending auth reconnect completion status: success=${success}, error=${errorMsg}`
    );
    if (mainWindowInstance?.webContents) {
      mainWindowInstance.webContents.send('mcp-auth-reconnect-complete', {
        serverId,
      ...
      console.warn(
        `[MCPManager][${serverId}] Could not send auth reconnect completion: mainWindow not available.`
      );
    }
  }
}
