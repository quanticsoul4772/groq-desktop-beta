...
async function handleExecuteToolCall(event, toolCall, discoveredTools, mcpClients, settings) {
  ...
  if (!toolCall || !toolCall.id || !toolCall.function || !toolCall.function.name) {
    ...
    return {
      error: 'Invalid tool call structure received from model.',
  ...
  try {
    ...
    if (!client) {
      console.error(
        `MCP Client instance not found for server ID: ${clientId} (required by tool ${toolName})`
      );
      return {
    ...
    try {
      ...
      console.log(
        `MCP tool "${toolName}" executed successfully. Result content length: ${JSON.stringify(result?.content)?.length}`
      );
      ...
      return {
        ...
        error: limitContentLength(
          `Error during execution of tool "${toolName}": ${executionError.message}`,
          settings.toolOutputLimit
        ),
        tool_call_id: toolCallId,
      };
    }
    ...
    console.error(
      `Unexpected error in handleExecuteToolCall for tool "${toolName}":`,
      handlerError
    );
    return {
      error: limitContentLength(
        `Internal error while handling tool call "${toolName}": ${handlerError.message}`
      ),
      tool_call_id: toolCallId,
    };
  }
