...
async function benchmarkJestPerformance() {
    ...
    for (const command of testCommands) {
        ...
        try {
            ...
            console.log(`   ${status} in ${coldResult.duration}ms (Jest: ${Math.round(coldResult.jestTime)}ms)`);
            
            if (coldResult.testCount > 0) {
                console.log(`   📊 ${coldResult.testCount} tests, ${Math.round(coldResult.jestTime / coldResult.testCount)}ms per test`);
            }
        } catch (error) {
        ...
        try {
            ...
            console.log(`   ${status} in ${warmResult.duration}ms (Jest: ${Math.round(warmResult.jestTime)}ms)`);
            
            if (warmResult.testCount > 0) {
                console.log(`   📊 ${warmResult.testCount} tests, ${Math.round(warmResult.jestTime / warmResult.testCount)}ms per test`);
            }
            
            // Calculate cache improvement
            const coldRun = results.find(r => r.command === command && r.runType === 'cold' && r.success);
            if (coldRun && warmResult.success) {
                const improvement = ((coldRun.jestTime - warmResult.jestTime) / coldRun.jestTime) * 100;
                const improvementTotal = ((coldRun.duration - warmResult.duration) / coldRun.duration) * 100;
                console.log(`   📈 Cache improvement: ${improvement.toFixed(1)}% (Jest), ${improvementTotal.toFixed(1)}% (total)`);
            }
        } catch (error) {
            console.error(`   ❌ ERROR: ${error.message}`);
    ...
    if (validComparisons > 0) {
        ...
        if (cacheExists) {
            try {
                ...
                console.log(`   Cache directory: ✅ Created (${stats.isDirectory() ? 'directory' : 'file'})`);
            } catch (error) {
...
if (require.main === module) {
    ...
    benchmarkJestPerformance().then(results => {
